...
from Project.finance_api.financeApi import FinanceApi
def do_TestUnit():
    print('--------------开始执行测试用例----------------')
    ...
    testSuit.addTest(unittest.defaultTestLoader.loadTestsFromTestCase(FinanceApi))
    #清空测试log与报告
    ...
    fp = open(filename, 'wb')
    #定义测试报告
    runner = HTMLTestRunnerCN.HTMLTestRunner(stream=fp, title='店达系统自动化测试报告',description='详细测试报告:')
    #运行测试用例
    runner.run(testSuit)
    fp.close()
